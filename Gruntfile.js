...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    /**
    * Sets constant for different environment and injects into app
    */
    ngconstant: {
      ...
      options: {
        space: '  ',
        wrap: '"use strict";\n\n {\%= __ngModule %}',
        name: require('./bower.json').name || 'dhsniem',
        dest: 'src/env.js',
        deps: false
      },
      // Environment targets. Dev = local. Production = remote (dev, test, prod).
      development: {
        constants: {
          SOLR_URL: '/solr/dhsniem/select',
          NODE_URL: '/api/jsonschema?'
        }
      },
      production: {
        constants: {
        ...
        files: {
          '<%= appSettings.app %>/index.html': [
            [
              ...
              '!{.tmp,<%= appSettings.app %>}/app/env.js',
              '!{.tmp,<%= appSettings.app %>}/app/service-worker.js',
    ...
    ngtemplates: {
      options: {
        ...
        module: require('./bower.json').name || 'dhsniem',
